...
+ (NSString *)hashOfData:(NSData *)dt withComputationContext:(FileHashComputationContext *)context {
    NSString *result = nil;
    //CFURLRef fileURL = CFURLCreateWithFileSystemPath(kCFAllocatorDefault, (CFStringRef)filePath, kCFURLPOSIXPathStyle, (Boolean)false);
    //CFReadStreamRef readStream = fileURL ? CFReadStreamCreateWithFile(kCFAllocatorDefault, fileURL) : NULL;
    //BOOL didSucceed = readStream ? (BOOL)CFReadStreamOpen(readStream) : NO;
    //if (didSucceed) {
        ...
        //    } else {
                ...
                uint8_t *buffer = (uint8_t *)[dt bytes];
                int readBytesCount = [dt length];
                (*context->updateFunction)(context->hashObjectPointer, (const void *)buffer, (CC_LONG)readBytesCount);
        //    }
        //}
...
+ (NSString *)md5HashOfData:(NSData *)dt {
    ...
    return [self hashOfData:dt withComputationContext:&context];
}
